...
def match_descriptors(directory_list, pids, target_desc, params):
    ...
    if 'paratope_db' not in params.keys():
        ...
        for index, ppi_pair_id in enumerate(directory_list):
            candidate_list = [k[0] for k in list(paratope_db.keys())]

            if ppi_pair_id not in candidate_list:
                continue

            mydescdir = os.path.join(params['seed_desc_dir'], ppi_pair_id)
            for pid in pids:
                name = (ppi_pair_id, pid)
                if name not in paratope_db:
                    continue
                fields = ppi_pair_id.split('_')
                try:
                    if pid == 'p1':
                ...
                parser = PDBParser()
                seed_pdb_path = os.path.join(params['seed_pdb_dir'],'{}.pdb'.format(pdb_chain_id))
                seed_struct = parser.get_structure(seed_pdb_path, seed_pdb_path)

                all_paratope_coords = []
                
                seed_cutoff = paratope_db[name]['cutoff']
                seed_atom_id = paratope_db[name]['atom_id']
                seed_paratope = paratope_db[name]['center_residue']
                seed_chain = paratope_db[name]['chain']

                for chain, paratope, atom in zip(seed_chain, seed_paratope, seed_atom_id):
                    seed_coord = seed_struct[0][chain][paratope][atom].get_coord()
                ...
                try:
                    seed_ply_fn = os.path.join(params['seed_ply_iface_dir'], pdb_chain_id+'.ply')
                    seed_mesh = pymesh.load_mesh(seed_ply_fn)
                except:
                    print("Error: Did not find ply file to load mesh in seed_ply_iface_dir for", pdb_chain_id)
                    continue
...
def multidock_source_and_check_binder(
    source_pcd, source_patch_coords, source_descs, 
    source_binder_pcd, source_binder_patch_coords, source_binder_descs, 
    cand_pts, target_pcd, target_descs,
    params):
    ...
    source_binder_ckdtree = cKDTree(np.asarray(source_binder_pcd.points))
    d, r = source_binder_ckdtree.query(np.asarray(source_pcd.points)[cand_pts], k=1)

    for pt1, pt2 in zip(cand_pts, r):
        source_patch, source_patch_descs, source_patch_idx = get_patch_geo(
            source_pcd, source_patch_coords, pt1, source_descs, outward_shift=params['surface_outward_shift'], flip_normals=True)
        
        source_binder_patch, source_binder_patch_descs, source_binder_patch_idx = get_patch_geo(
            source_binder_pcd, source_binder_patch_coords, pt2, source_binder_descs, outward_shift=params['surface_outward_shift'], flip_normals=False)
        ...
        # source_patch.transform(result_icp.transformation)
        source_binder_patch.transform(result_icp.transformation)
        all_results.append(result_icp)
        all_source_patch.append(source_binder_patch)
        all_source_desc.append(source_binder_patch_descs)
        all_source_idx.append(source_binder_patch_idx)
        
        s_ckdtree = cKDTree(np.array(target_pcd.points))
        a, b = s_ckdtree.query(np.array(source_binder_patch.points))
        all_corr_num.append(b[np.where(a<=1.0)[0]])

    return all_results, all_source_patch, all_source_desc, all_source_idx, all_corr_num
...
def align_protein_v2(
    ...
    if source_pid == 'p1':
        source_binder_pid = 'p2'
        source_name = ppi_pair_id.split('_')[0]+'_'+ppi_pair_id.split('_')[1]
        source_chain = ppi_pair_id.split('_')[1]
        source_chain_number = 1
        source_binder_chain_number = 2
    else: 
        source_binder_pid = 'p1'
        source_name = ppi_pair_id.split('_')[0]+'_'+ppi_pair_id.split('_')[2]
        source_chain = ppi_pair_id.split('_')[2]
        source_chain_number = 2
        source_binder_chain_number = 1

    with open(params['paratope_db'], 'rb') as f:
        paratope_db = pickle.load(f)

    source_binder_name = paratope_db[name]['targets'][0]
    source_binder_chain = paratope_db[name]['targets'][0].split('_')[1]    
    ...
    if sim_search:
        source_pcd, source_desc, source_iface = load_protein_pcd(ppi_pair_id, source_chain_number, source_paths, flipped_features=True, read_mesh=False)

        source_patch_coords =  get_patch_coords(params['seed_precomp_dir'], ppi_pair_id, source_pid, cv=source_vix)
        source_binder_pcd, source_binder_desc, source_binder_iface = load_protein_pcd(ppi_pair_id, source_binder_chain_number, source_paths, flipped_features=False, read_mesh=False)

        source_binder_ckdtree = cKDTree(np.asarray(source_binder_pcd.points))
        d, r = source_binder_ckdtree.query(np.asarray(source_pcd.points)[source_vix], k=1)

        source_binder_patch_coords =  get_patch_coords(params['seed_precomp_dir'], ppi_pair_id, source_binder_pid, cv=r)

        # Perform all alignments to target. 
        all_results, all_source_patch, all_source_patch_desc, all_source_idx, all_corr_num = multidock_source_and_check_binder(
                source_pcd, source_patch_coords, source_desc, 
                source_binder_pcd, source_binder_patch_coords, source_binder_desc, 
                source_vix, target_patch, target_patch_descs, 
                params
                ) 
    else: 
        source_pcd, source_desc, source_iface = load_protein_pcd(ppi_pair_id, source_chain_number, source_paths, flipped_features=False, read_mesh=False)
        ...
        all_results, all_source_patch, all_source_patch_desc, all_source_idx = multidock(
                source_binder_pcd, source_binder_patch_coords, source_binder_desc, 
                r, target_patch, target_patch_descs, 
                params
                ) 


    # Score the results using a 'lightweight' scoring function.
    all_source_scores = [None]*len(r)
    for viii in range(len(r)):
        if sim_search:
            corr = all_corr_num[viii]
        else:
            corr = all_results[viii].correspondence_set
        if len(corr)/float(len(np.asarray(all_source_patch[viii].points))) < 0.3:
    ...
    source_outdir = os.path.join(site_outdir, '{}'.format(source_binder_name))
    ...
    for j, k in enumerate(r):
        ...
        source_struct = parser.get_structure('{}'.format(source_binder_name), os.path.join(params['seed_pdb_dir'],'{}.pdb'.format(source_binder_name)))
        ...
        if clashing_ca <= params['allowed_CA_clashes'] and clashing_total <= params['allowed_heavy_atom_clashes']:        # Align and save the pdb + patch 
            out_fn = source_outdir+'/{}_{}'.format(source_binder_name, k)
