...
target_pcd, target_desc, target_iface, target_mesh = load_protein_pcd(target_ppi_pair_id, target_chain_ix,
...
if 'target_residue' in params:
    ...
    target_resid = (' ', int(params['target_residue']['resid']), ' ')
...
for site_ix, site_vix in zip(site_ixs, site_vixs):
    print('Starting site near residue {}\n'.format(params['target_residue']['resid']))
    site_outdir = os.path.join(outdir, 'res_{}_site_{}'.format(params['target_residue']['resid'], site_vix))
